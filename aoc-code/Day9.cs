...
namespace aoc_code
{
    public class Day9
    {
        ...
        {
            var inputs = input.Split("\r\n").Select(l => Int64.Parse(l)).ToArray();

            return this.FindWeakness(inputs, size);
        }
        ...
        public long Run2(string input, int size)
        {
            ...
            var weakness = this.FindWeakness(inputs, size);

            var interestingArray = inputs.FindSubSetThatSumsTo(weakness);

            if(interestingArray != null)
            {
                return interestingArray.Min() + interestingArray.Max();
            }
        ...
        private long FindWeakness(long[] inputs, int size)
        {
            ...
            while(dataIndex < inputs.Length ){
                ...
                if(!preambleData.HasTwoItemsThatSumsTo(currentNumber))
                {
    ...
    public static class ArrayExtensions
    {
        ...
        public static long[] FindSubSetThatSumsTo(this long[] array, long target)
        {
            for(var size = 2; size < array.Length; size++)
            {
                ...
                {
                    var trying = array.Skip(startIndex).Take(size);
                    var sum = trying.Sum();
                    if(sum == target){
                        return trying.ToArray();
                    }
                }
            }
            return null;
